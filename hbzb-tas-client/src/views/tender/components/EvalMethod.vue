            ...
            <div class="form-box" style="width:600px; margin:10px auto" v-loading="loading">
                <el-form ref="form" :model="form" :rules="rules" label-width="130px">
                    ...
                    <el-form-item label="基准值公式" prop="basicFormula">
                        <el-input v-model="form.basicFormula">
                            <el-button slot="append" icon="el-icon-search"></el-button>
                        </el-input>
                    </el-form-item>
                    <el-form-item label="扣分公式" prop="deductFormula">
                        <el-input v-model="form.deductFormula">
                            <el-button slot="append" icon="el-icon-search"></el-button>
                        </el-input>
                    </el-form-item>
...
<script>
    export default {
        ...
        created() {
            if(this.section.evalMethod) {
                this.evalMethod = this.section.evalMethod;
                this.fetchData();
            }
        },
        watch: {
        ...
        methods: {
            ...
            fetchData() {
                this.loading = true;
                this.$http.get('/tas/eval/criteria/list?sectionUid=' + this.section.uid + '&evalMethod=' + this.section.evalMethod).then((data) => {
                    this.tableData = data;
                    this.loading = false;
                    this.visible = true;
            ...
            initMethod() {
                this.loading = true;
                this.$http.get('/tas/eval/criteria/init?sectionUid=' + this.section.uid + '&evalMethod=' + this.evalMethod).then((data) => {
                    this.loading = false;
                    this.section.evalMethod = this.evalMethod; //section
                    sessionStorage.setItem('section', JSON.stringify(this.section));
                    this.fetchData();
                }).catch(() => this.loading = false);
            },
            // 选择评标方法
            ...
            createCriteria(parentCode) {
                ...
                this.form = {};
                this.form.sectionUid = this.section.uid;
