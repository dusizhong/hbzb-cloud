<template>
	<div v-loading="loading">
		...
		<div class="container">
			...
			<el-alert v-if="showTip" type="warning" show-icon :title="tip" style="margin-bottom:20px"></el-alert>

			<div style="display:-webkit-flex; display:flex; margin-bottom:20px; padding:15px; background:#F2F6FC">
				<div style="flex-grow:1; text-align:center; color:#303133; font-weight:bold">单位信息<span style="font-size:14px">（未审核）</span></div>
				<!-- <div style="color:#E6A23C;font-size:14px">未审核</div> -->
			</div>
						...
						<el-row :gutter="20">
							<el-col :span="12">
								<el-form-item label="单位名称" prop="name">
									<div v-if="disabled" style="font-weight:bold">{{form.name}}</div>
									<el-input v-else v-model="form.name" placeholder="请填写与营业执照一致的单位全称" :disabled="disabled"></el-input>

								</el-form-item>
							</el-col>
							<el-col :span="12">
								<el-form-item label="统一社会信用代码" prop="uniformCode">
									<div v-if="disabled" style="font-weight:bold">{{form.uniformCode}}</div>
									<el-input v-else v-model="form.uniformCode" placeholder="请填写" :disabled="disabled"></el-input>
								</el-form-item>
							</el-col>
						</el-row>
						<el-row :gutter="20">
							<el-col :span="12">
								<el-form-item label="所属辖区" prop="area">
									<div v-if="disabled" style="font-weight:bold">{{form.area}}</div>
									<el-cascader v-else v-model="form.area" :options="areas" style="width:100%" :disabled="disabled"></el-cascader>
								</el-form-item>
							</el-col>
							<el-col :span="12">
								<el-form-item label="国别/地区" prop="nation">
									<div v-if="disabled" style="font-weight:bold">{{form.nation}}</div>
									<el-select v-else v-model="form.nation" style="width:100%" placeholder="请选择" :disabled="disabled">
						...
						<el-row :gutter="20">
							<el-col :span="12">
								<el-form-item label="单位类型" prop="type">
									<div v-if="disabled" style="font-weight:bold">{{form.type}}</div>
									<el-select v-else v-model="form.type" style="width:100%" placeholder="请选择" :disabled="disabled">
							...
							<el-col :span="12">
								<el-form-item label="单位性质" prop="attribute">
									<div v-if="disabled" style="font-weight:bold">{{form.attribute}}</div>
									<el-cascader v-else v-model="form.attribute" :options="attributes" style="width:100%" placeholder="请选择" :disabled="disabled"></el-cascader>
								</el-form-item>
							</el-col>
						</el-row>
						<el-row :gutter="20">
	...
	export default {
		...
		methods: {
			...
			handleBeforeUpload(res, row) {
				...
				console.log(JSON.stringify(this.uploadParams))
			...
			onSubmit() {
				...
				this.loading = false;
					...
					this.disabled = true;
					this.tip = "信息已提交，我们将在2个工作日内为您审核。";
					this.showTip = true;
					this.$message.success('提交成功！');
					this.$router.replace('/UserCorpDetail');
